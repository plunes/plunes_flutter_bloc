...
class WebServices {
  ...
  Future<CatalogueList> getCatalogue(BuildContext context) async {
    dynamic postsList = await _apiCall.getAPIRequest(context, urls.catalogue, '', false);
    return CatalogueList.fromJson(postsList);
  }

  Future<dynamic> getUserExistence(BuildContext context, String phoneNumber) async {
    dynamic postsList = await _apiCall.getAPIRequest(context, urls.checkUserExistence+phoneNumber, '', false);
    return postsList;
  }
  Future<LoginPost> getProfileRequest(BuildContext context, String token) async {
    return LoginPost.fromJson(await _apiCall.getAPIRequest(context, urls.whoAmI, '', false, token: token));
  }
  ...
  Future<dynamic> postChangePassword(BuildContext context, phone, password) async {
    var body = {'mobileNumber': phone,'password': password};
    return await _apiCall.getAPIRequest(context, urls.changePassword, '1', false, body: json.encode(body), method: Constants.PUT);
  }
  Future<dynamic> postHelpResult(BuildContext context, details, token) async {
    var body = {'text': details};
    return await _apiCall.getAPIRequest(context, urls.help, '1', false, body: json.encode(body),token: token, method: Constants.POST);
  }
  Future<AllNotificationsPost> postNotificationResult(BuildContext context, token) async {
    dynamic result= await _apiCall.getAPIRequest(context, urls.notification, '', false, token: token);
    print(result.toString());
    return AllNotificationsPost.fromJson(result);
  }

  Future<LoginPost> postRegistrationRequest(BuildContext context, var body) async {
    dynamic postsList = await _apiCall.getAPIRequest(context, urls.signUp, '1', false, body: json.encode(body), method: Constants.POST);
    return LoginPost.fromJson(postsList);
  }
