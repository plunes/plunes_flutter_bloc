...
class HospitalProfileScreenState extends State<HospitalProfileScreen> with TickerProviderStateMixin,ImagePickerListener implements DialogCallBack{
  ...
  var globalHeight, globalWidth,imageUrl='',    _latitude = '', _bannerImageUrl='', _longitude = '';
  var  _speciality='',_userName = '', _userType = '', _userEmail = '', _accessToken = '', _phoneNo = '', _userLocation = '', _userDOB = '', _userEducation = '', _userCollege, _profRegNo, _practising, _experience, _introduction, _gender;
...
//                pinned: _appBarBehavior == AppBarBehavior.pinned,
                ...
                background: Stack(
                  ...
                  children: <Widget>[
                    Stack(
                      children: <Widget>[
                        InkWell(onTap: (){
                          widget.showPhoto(context, Photo(assetName: _bannerImageUrl, title: '', caption: ''));
                        },child: Container(
                          height: 250.0,
                          width: globalWidth,
                          child: _bannerImageUrl.contains('http')? Image.network(_bannerImageUrl, fit: BoxFit.cover,): Image.asset(_bannerImageUrl, fit: BoxFit.cover),
                          decoration: BoxDecoration(
                            color: Colors.transparent,
                            image: DecorationImage(
                              fit: BoxFit.cover,
                              image: AssetImage(assetsImageFile.gradientImageArray[6]),
                            ),
                          ),
                        ),),
                        InkWell(onTap: (){
                          ...
                          imagePicker.showDialog(context);
                            ...
                            margin: EdgeInsets.only(top: (10), left: globalWidth - 60),
                            ...
                            child: Column(
                              ...
                              children: <Widget>[
                                ...
                                Row(
                                  ...
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: <Widget>[
                                    Expanded(child: getProfileInfoView(25, 22, assetsImageFile.locationIcon, stringsFile.locationSep, _userLocation)),
                                    editButton('1')
                                  ],)
                                ,
                                ...
                                Row(
                                  ...
                                  children: <Widget>[
                                    Expanded(child:widget.createTextViews(stringsFile.introduction, 15, colorsFile.black0, TextAlign.start, FontWeight.normal),),
                                    editButton('2')
                                  ],),
                                ...
                                widget.createTextViews(stringsFile.specialization, 15, colorsFile.black0, TextAlign.start, FontWeight.normal),
                                widget.getSpacer(0.0, 10.0),
                                Container(
                                  decoration: BoxDecoration(borderRadius: BorderRadius.all(Radius.circular(10)), border: Border.all(width: 1, color: Color(0xff01d35a))),
                                  child: Padding(
                                    padding: const EdgeInsets.all(8.0),
                                    child: DropdownButtonFormField(
                                      value: _speciality,
                                      decoration: InputDecoration.collapsed(hintText: stringsFile.chooseSpeciality, hintStyle: TextStyle(color: Colors.black), hasFloatingPlaceholder: true, fillColor: Colors.white),
                                ...
                                Row(
                                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                  children: <Widget>[
                                    Expanded(child:widget.createTextViews(stringsFile.teamOfExperts, 15, colorsFile.black0, TextAlign.start, FontWeight.normal),),
                                    editButton('3')
                                  ],),
        ...
        children: <Widget>[
          Expanded(
              child: Row(
                ...
                mainAxisAlignment: MainAxisAlignment.center,
                children: <Widget>[
                  Container(width: width, height: height, margin: EdgeInsets.only(right: 8), child: widget.getAssetIconWidget(icon, height, width, BoxFit.contain)),
                  Expanded(child: widget.createTextViews(title, 14, colorsFile.black0, TextAlign.start, FontWeight.normal))
                ],
              )),
          Expanded(
              child: Container(
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.hasData) {
                        return Container(child: CircleAvatar(
                          radius: 30,
                          backgroundImage: snapshot.data.toString().contains('http')?NetworkImage(snapshot.data.toString()): ExactAssetImage(snapshot.data.toString()),
                        ));
                      } else
  ...
  Widget editProfileBackButton() {
    return Container(
        ...
        child: Stack(
          children: <Widget>[
            CircleAvatar(
              backgroundColor: Color(CommonMethods.getColorHexFromStr(colorsFile.defaultGreen)),
              radius: 15,
            ),
            Center(
                child: Icon(
  ...
  void initialize()  {
    ...
    bloc.profileData.listen((data) async {
      if(data.success)  {
        ...
        await bloc.saveDataInPreferences(data, context, null);
        widget.showInSnackBar(stringsFile.somethingWentWrong, Colors.red, _scaffoldKey1);
      }
    }, onDone: () {
      bloc.disposeProfileStream();
    });
  ...
  Future<dynamic> getSharedPreferenceData() async {
    ...
    _bannerImageUrl = preferences.getPreferenceString(Constants.PREF_USER_BANNER_IMAGE);
...
//    _userType = Constants.hospital;
    ...
    return  Container(
      child: InkWell(
        ...
        child: Container(
            ...
            child: widget.createTextViews(_for=='3'?stringsFile.add:stringsFile.edit, 15, colorsFile.defaultGreen, TextAlign.start, FontWeight.normal)),
      ),
    );
  }
  ...
  fetchLocation() {
    Navigator.of(context).push(PageRouteBuilder(opaque: false, pageBuilder: (BuildContext context, _, __) => LocationFetch())).then((val) {
      ...
      var address = addressControllerList[0] + ' ' + addressControllerList[1] + ' ' + addressControllerList[2];
