...
import 'package:plunes/models/whatsapp_login/whatsapp_token.dart';
...
//TODO: handle whatsapp not found
  ...
  Future<void> initPlatformState() async {
    _otplessFlutterPlugin.authStream.listen((token) {
      setState(() {
        var _waId = token ?? "Unknown";

        print("resultresult--:>${_waId}");
        generateWhatsappToken(_waId);
...
//      }
  ...
  _whatsappLogin() {
    if (phoneController.text.isEmpty)
      _showInSnackBar("Please enter whatsapp Number", PlunesColors.BLACKCOLOR,
          _scaffoldKey);
    else if (!_isNumber() || phoneController.text.trim().length != 10)
      _showInSnackBar(plunesStrings.invalidPhoneNumber, PlunesColors.BLACKCOLOR,
          _scaffoldKey);
    else {
      _showInSnackBar("else", PlunesColors.BLACKCOLOR, _scaffoldKey);
      //  _userLoginRequest();
    }
  }
  ...
  _userWhatsappLoginRequest(var phoneNumber) async {
    progress = true;
    _setState();
    await Future.delayed(Duration(milliseconds: 100));
    var result = await _userBloc!.whatsappLogin(phoneNumber);
    progress = false;
    _setState();
    await Future.delayed(Duration(milliseconds: 100));
    print("data-->${result.toString()}");
  ...
  void generateWhatsappToken(var verificationCode) async {
    print("WhatsappToken_auth : called");

    var headers = {
      'clientId': 'om0hm9vz',
      'clientSecret': 'k1vlvcnx6duek166',
      'Content-Type': 'application/json'
    };
    var request = http.Request('POST', Uri.parse('https://plunes.authlink.me'));
    request.body = json.encode({
      "waId": "$verificationCode"
    });
    request.headers.addAll(headers);

    http.StreamedResponse response = await request.send();
    print("WhatsappToken_auth : started");

    if (response.statusCode == 200) {
      print("WhatsappToken_auth : 200_status_code");

      WhatsappToken wResponse = WhatsappToken.fromJson(jsonDecode(await response.stream.bytesToString()));
      print("WhatsappToken_auth : ${wResponse.statusCode}");
      print("WhatsappToken_auth : ${wResponse.status}");
      print("WhatsappToken_auth : ${wResponse.success}");
      print("WhatsappToken_auth_mob : ${wResponse.data!.userMobile}");

      var mobile = wResponse.data!.userMobile;
      if(mobile.toString().length > 10) {
       var mobile_ = mobile!.substring((mobile!.length-10), mobile!.length);
        print("mobile_if::$mobile_");
       _userWhatsappLoginRequest(mobile_);

      } else {
        print("mobile_else::${wResponse.data!.userMobile}");
        _userWhatsappLoginRequest(wResponse.data!.userMobile);
      }
    }
    else {
      print("WhatsappToken_auth_error:${response.reasonPhrase}");
      print("generateWhatsappToken_error:${response.reasonPhrase}");
      _showInSnackBar(response.reasonPhrase, PlunesColors.BLACKCOLOR, _scaffoldKey);
    }
  }
